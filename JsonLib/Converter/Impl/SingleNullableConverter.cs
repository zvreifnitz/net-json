...
{
    internal sealed class SingleNullableConverter : ConverterBase<float?>
    {
        ...
        public override void FromJson(IJsonSerializators context, IJsonReader reader, ref float? instance)
        {
            ...
            if (token == JsonToken.Null)
            {
                instance = null;
                     ...
                     float.TryParse(reader.ReadValue(), FloatNumberStyle, DefaultCultureInfo, out float parsed))
            {
                instance = parsed;
            }
            else
        ...
        public override float? NewInstance()
        {
            return null;
        }
    }
