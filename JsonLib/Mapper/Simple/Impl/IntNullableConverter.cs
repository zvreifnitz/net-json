...
{
    internal sealed class IntNullableConverter : ConverterBase<int?>
    {
        ...
        public override void FromJson(IJsonSerializators context, IJsonReader reader, out int? instance)
        {
            ...
            if (token == JsonToken.Null)
            {
                instance = null;
                     ...
                     int.TryParse(reader.ReadValue(), IntegerNumberStyle, DefaultCultureInfo, out int parsed))
            {
                instance = parsed;
            }
            else
            {
                instance = ThrowInvalidJsonException<int?>();
            }
        }
    }
}