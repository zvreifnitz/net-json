...
namespace com.github.zvreifnitz.JsonLib.Mapper.Collection
{
    ...
    internal sealed class QueueMapperBuilder : IJsonMapperBuilder
    {
        private static readonly MethodInfo CreateMapperMethod =
            typeof(QueueMapperBuilder).GetRuntimeMethod("CreateMapper", new[] {typeof(IJsonSerializators)});

        private static readonly Type ListType = typeof(Queue<>);
        ...
        public IJsonMapper<Queue<T>> CreateMapper<T>(IJsonSerializators context)
        {
            try
            {
            ...
            {
                return ExceptionHelper.ThrowUnexpectedException<IJsonMapper<Queue<T>>>(exc);
            }
        }

        private sealed class ListMapper<T> : IJsonMapper<Queue<T>>
        {
            ...
            public void ToJson(IJsonSerializators context, IJsonWriter writer, Queue<T> instance)
            {
                if (instance == null)
                {
            ...
            public Queue<T> FromJson(IJsonSerializators context, IJsonReader reader)
            {
                ...
                {
                    return ExceptionHelper.ThrowInvalidJsonException<Queue<T>>();
                }
                Queue<T> result = new Queue<T>();
                while (true)
                {
                    result.Enqueue(FromJsonItem(context, reader));
