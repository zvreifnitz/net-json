...
namespace com.github.zvreifnitz.JsonLib.Mapper.Collection
{
    ...
    internal sealed class ItfListMapperBuilder : IJsonMapperBuilder
    {
        private static readonly MethodInfo CreateMapperMethod =
            typeof(ItfListMapperBuilder).GetRuntimeMethod("CreateMapper", new[] {typeof(IJsonSerializators)});
        ...
        public IJsonMapper<IList<T>> CreateMapper<T>(IJsonSerializators context)
        {
            try
            {
            ...
            {
                return ExceptionHelper.ThrowUnexpectedException<IJsonMapper<IList<T>>>(exc);
            }
        }

        private sealed class ListMapper<T> : IJsonMapper<IList<T>>
        {
            ...
            public void ToJson(IJsonSerializators context, IJsonWriter writer, IList<T> instance)
            {
                if (instance == null)
                {
            ...
            public IList<T> FromJson(IJsonSerializators context, IJsonReader reader)
            {
                ...
                {
                    return ExceptionHelper.ThrowInvalidJsonException<IList<T>>();
                }
