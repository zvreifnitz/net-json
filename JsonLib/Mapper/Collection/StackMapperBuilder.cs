...
namespace com.github.zvreifnitz.JsonLib.Mapper.Collection
{
    ...
    internal sealed class StackMapperBuilder : IJsonMapperBuilder
    {
        private static readonly MethodInfo CreateMapperMethod =
            typeof(StackMapperBuilder).GetRuntimeMethod("CreateMapper", new[] {typeof(IJsonSerializators)});

        private static readonly Type ListType = typeof(Stack<>);
        ...
        public IJsonMapper<Stack<T>> CreateMapper<T>(IJsonSerializators context)
        {
            try
            {
            ...
            {
                return ExceptionHelper.ThrowUnexpectedException<IJsonMapper<Stack<T>>>(exc);
            }
        }

        private sealed class ListMapper<T> : IJsonMapper<Stack<T>>
        {
            ...
            public void ToJson(IJsonSerializators context, IJsonWriter writer, Stack<T> instance)
            {
                if (instance == null)
                {
            ...
            public Stack<T> FromJson(IJsonSerializators context, IJsonReader reader)
            {
                ...
                {
                    return ExceptionHelper.ThrowInvalidJsonException<Stack<T>>();
                }
                Stack<T> result = new Stack<T>();
                while (true)
                {
                    result.Push(FromJsonItem(context, reader));
